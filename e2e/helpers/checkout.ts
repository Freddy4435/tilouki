...
export async function openCheckoutFromCart(page: Page) {
  ...
  await page.getByLabel("Prénom", { exact: true }).waitFor({ state: "visible", timeout: 20_000 });
}
...
export async function selectMockRelayPoint(page: Page) {
  ...
  await page.getByRole("button", { name: /payer en toute sécurité/i }).waitFor({ state: "visible" });
}

export async function assertCgvBlocksPayment(page: Page) {
  await page.getByRole("button", { name: /payer en toute sécurité/i }).click();
  await page
    .getByText(/vous devez accepter les conditions générales de vente/i)
