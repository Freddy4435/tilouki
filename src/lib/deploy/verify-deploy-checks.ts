 ...
 * scripts/lib/verify-deploy-checks.mjs et docs/PRODUCTION_CHECKLIST.md
 */
...
export type DeployCheckLevel = "error" | "warn" | "ok";

export interface DeployCheck {
  id: string;
  level: DeployCheckLevel;
  message: string;
  fix?: string;
}
...
export function resolveMondialRelayBrandId(
...
export function runProductionEnvChecks(
  envVars: Record<string, string | undefined>,
): DeployCheck[] {
  ...
  for (const key of ["FROM_EMAIL", "ADMIN_EMAIL"] as const) {
    const value = envVars[key]?.trim() ?? "";
...
function fixHintForKey(key: string): string {
...
export function verifyStripeWebhookEventsInSource(source: string): DeployCheck[] {
  const checks: DeployCheck[] = [];
...
export function verifyGitignoreCoversSecrets(gitignore: string): DeployCheck[] {
  const checks: DeployCheck[] = [];
  const expected = [
